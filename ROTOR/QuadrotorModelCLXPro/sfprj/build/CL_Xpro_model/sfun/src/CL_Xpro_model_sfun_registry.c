...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_CL_Xpro_model_process_check_sum_call( int nlhs, mxArray
   * plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_CL_Xpro_model_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_autoinheritence_call( int nlhs, mxArray * plhs[], int
 nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_CL_Xpro_model_autoinheritance_info( int nlhs, mxArray *
   plhs[], int nrhs, const mxArray * prhs[] );
  if(sf_CL_Xpro_model_autoinheritance_info(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
...
extern unsigned int sf_CL_Xpro_model_method_dispatcher(SimStruct *S, const char
...
{
  if(sf_CL_Xpro_model_method_dispatcher(simstructPtr,chartName,method,data))
  return 1;
  return 0;
}
extern void CL_Xpro_model_terminator(void);
void sf_machine_global_terminator(void)
{
  if(sfMachineGlobalTerminatorCallable) {
    ...
    CL_Xpro_model_terminator();
    sf_debug_terminate();
  }
  return;
}
extern void CL_Xpro_model_initializer(void);
extern void CL_Xpro_model_register_exported_symbols(SimStruct* S);
extern void CL_Xpro_model_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  CL_Xpro_model_register_exported_symbols(S);
}
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if(sfMachineGlobalInitializerCallable) {
    ...
    if(!simModeIsRTWGen) {
      CL_Xpro_model_debug_initialize();
    }
    CL_Xpro_model_initializer();
  }
  return;
}
